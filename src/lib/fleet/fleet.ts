...
export function buildDriverRow(
  ...
  return { driver, gross, cash, vat, appFee, weeks, payout: gross - appFee - cash - vat };
}
...
<div class="box">
  <table>
    ${line("Gross earnings", money(row.gross))}
    ${line("Cash collected", `− ${money(row.cash)}`)}
    ${line("VAT / fleet commission", `− ${money(row.vat)}`)}
    ${line(`Application fee (${row.weeks} week${row.weeks === 1 ? "" : "s"})`, `− ${money(row.appFee)}`)}
  </table>
  <div class="total"></div>
  <table>${line("Payout", money(row.payout), true)}</table>
</div>
