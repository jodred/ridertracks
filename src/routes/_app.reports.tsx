...
function ReportsPage() {
  ...
  const exportPDF = () => {
    const rowsHtml = summary.daily
      ...
      .join("");
    const html = `<!doctype html><html><head><title>RideTracks Report ${range.from} to ${range.to}</title>
...
<h1>RideTracks Report</h1><small>${range.from} to ${range.to} - ${state.fleet.fleetName}</small>
<h3>Summary</h3>
<table>
